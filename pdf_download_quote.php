...
$link = mysqli_connect('localhost','root','','ringsecurev2db') or die('Cannot connect to the DB');
     ...
     if (!$link) {
        ...
        $sql = "SELECT * FROM `tbl_voicemax_quotes`  WHERE `id` = '".$id."' ";
        $check = mysqli_query($link,$sql);
        $quotes = mysqli_fetch_assoc($check);  
        ...
        $sql = "SELECT * FROM `tbl_voicemax_quotes_services` WHERE `quote_id` = '".$id."'";
        $check = mysqli_query($link,$sql);
        $product_list = mysqli_fetch_assoc($check); 
        
        $sql = "SELECT `staff_name` FROM `tbl_voicemax_staff_users` WHERE `staff_id` = '".$quotes['sales_person']."'";
        $check = mysqli_query($link,$sql);
        $sales_person = mysqli_fetch_assoc($check); 

        if($quotes['payment_terms']=='1'){
            $payment_terms = 'On delivery';
        }elseif($quotes['payment_terms']=='2'){
            $payment_terms = 'Advance';
        }elseif($quotes['payment_terms']=='3'){
            $payment_terms = 'Online';
        }elseif($quotes['payment_terms']=='4'){
        ...
        if($action == '1'){
               $html = '<!doctype html>
                    ...
                    <tr>
                        ...
                        <td style="text-align: right;">
                            Quote #: '.$quotes['reference_number'].'<br>
                            Created Date: '.$quotes['quote_date'].'<br>
                            Expiry Date: '.$quotes['expiry_date'].'<br>
                            Due Date: '.$quotes['due_date'].'
                        </td>
                    </tr>
                ...
                <table class="products" cellpadding="3" cellspacing="3">
                    ...
                    <tr>
                        <td>
                            ...
                            '.$quotes['bill_address'].'<br>
                            '.$quotes['bill_city'].'<br>
                            '.$quotes['bill_state'].'  '.$quotes['bill_zipcode'].'
                        </td>
                        
                        <td style="text-align: right;">
                        
                            '.$quotes['ship_address'].'<br>
                            '.$quotes['ship_city'].'<br>
                            '.$quotes['ship_state'].'  '.$quotes['ship_zipcode'].'
                        </td>
                    </tr>
            ...
            $filename = $quotes['reference_number'].'-'.$client['client_name'].'-'.date('m-d-Y');
            $dompdf = new DOMPDF();
