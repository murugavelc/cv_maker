...
$link = mysqli_connect('localhost','root','','ringsecurev2db') or die('Cannot connect to the DB');
     ...
     if (!$link) {
        ...
        $sql = "SELECT * FROM `tbl_voicemax_orders` WHERE `id` = '".$id."'";
        $check = mysqli_query($link,$sql);
        $orders = mysqli_fetch_assoc($check);  
        
        $sql = "SELECT * FROM `tbl_voicemax_crm_clients` WHERE `created_by` = '".$logged_id."' AND `status` = 1";
        $check = mysqli_query($link,$sql);
        ...
        $sql = "SELECT * FROM `tbl_voicemax_orders_services` WHERE `quote_id` = '".$id."'";
        $check = mysqli_query($link,$sql);
        $product_list = mysqli_fetch_assoc($check); 
        
        $sql = "SELECT `staff_name` FROM `tbl_voicemax_staff_users` WHERE `staff_id` = '".$orders['sales_person']."'";
        $check = mysqli_query($link,$sql);
        $sales_person = mysqli_fetch_assoc($check); 

        if($orders['payment_terms']=='1'){
            $payment_terms = 'On delivery';
        }elseif($orders['payment_terms']=='2'){
            $payment_terms = 'Advance';
        }elseif($orders['payment_terms']=='3'){
            $payment_terms = 'Online';
        }elseif($orders['payment_terms']=='4'){
       ...
       // print_r($product_items); die;
        if($action == '1'){
            ...
            $filename = $orders['id'].'-'.$client['client_name'].'-'.date('m-d-Y');
            $dompdf = new DOMPDF();
